...
// ForcedAuthRequest represents a forced authorization transaction request
type ForcedAuthRequest struct {
	...
	// ReferenceOrderID is the reference order ID for the forced authorization transaction. Unique ID assigned by merchant system to identify this forced authorization transaction, 6-32 characters, can only contain numbers, uppercase/lowercase letters, _-\|*
	ReferenceOrderID string `json:"referenceOrderId"`

	// TransactionRequestID is the transaction request ID for this forced authorization transaction. Unique ID to identify this forced authorization transaction request, used as API idempotency control field
